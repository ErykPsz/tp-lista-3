...
public abstract class CompositeShape extends Shape {
	...
	public int getShapeCount() {
		...
		for (Shape shape : this.shapes) {
			...
			CompositeShape compositeChild = shape.asComposite();

			if (compositeChild != null) {
				n += compositeChild.getShapeCount();
			}
		}
		return n;
	...
	public List<Shape> getShapesByType(ShapeType shapeType) {
		...
		for (Shape shape : this.shapes) {
			...
			CompositeShape composite = shape.asComposite();
			if (composite != null) {
				result.addAll(composite.getShapesByType(shapeType));
			}
		}
	...
	public List<Shape> getLeafShapes() {
		...
		for (Shape shape : shapes) {
			...
			if (composite == null) {
				...
				// to composite – zbieramy liście z jego wnętrza
				result.addAll(composite.getLeafShapes());
			}
		}
